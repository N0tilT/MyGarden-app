...
class PlantVarietyRemoteDataSource
    extends RemoteDataSource<List<PlantVarietyModel>, GardenRequestModel> {
  final http.Client client;

  PlantVarietyRemoteDataSource({required this.client});

  @override
  Future<List<PlantVarietyModel>> load(
    GardenRequestModel request,
    String token,
  ) async {
    try {
      final uri = Uri.parse('$BASE_URL/plant_variety').replace(
        queryParameters: {
      ...
      if (response.statusCode == 200) {
        ...
        final responseModel = decoded
            .map(
              (dynamic value) =>
                  PlantVarietyModel.fromJson(value as Map<String, dynamic>),
            )
            .toList();
